...
def get_data():
    ...
    df_top["year"] = pd.to_datetime(df_top["release_date"], errors="coerce").apply(
    ...
    return (
        df_top[["title", "year", "vote_count", "vote_average", "genres", "overview"]],
        df,
        cos_sim,
    )
...
try:
    n = st.number_input(
        "Insert a number of movies", min_value=1, max_value=100, value=10
    )
    ...
    if not genre:
        ...
        data = st.table(
            r.top_n_recommender(n, df_top, genre).drop(
                ["genres", "wr", "vote_count", "vote_average"], axis=1
            )
        )

    films = df["title"]
    film = st.selectbox("Choose title:", films)
    if not film:
        ...
        data2 = st.table(
            pd.DataFrame(r.nlp_based_recommenderer(n, df, str(film), cos_sim))
            .merge(df_top, on="title")
            .drop(["genres", "vote_count", "vote_average"], axis=1)
        )
